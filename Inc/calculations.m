...
W = Nsw/2 * W_u_d .* W_delay .* W_PWM .* LPF(frequencyHz, R1, R2, R3, R4, C1, C2);
%W = W_u_i .* LPF(frequencyHz, R1, R2, R3, R4, C1, C2);

load('simulation\W_u_d_LPF_24V_Full_load.mat')
...
for idx = 1 : NUM_OF_DATASETS
    switch idx
        case 1
            ...
            frDatasets{idx}(:,varIndex.complex_mag) = W;
            frDatasets{idx}(:,varIndex.frequency  ) = frequencyHz;
        case 2
            frDatasets{idx} = W_u_d_LPF_24V_Full_load;
    end
end
